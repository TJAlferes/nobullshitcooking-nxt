...
import Friends from '../../../src/pages/friends/index.page';
...
jest.spyOn(require("react-redux"), "useDispatch").mockReturnValue(mockedDispatch);

jest.spyOn(require("next/router"), "useRouter").mockImplementation(() => ({route: "/friends", pathname: "/friends"}));
...
describe('Friends', () => {
  it('should record and display changes to the user to find', () => {
    wrapper.find('input[name="friends-find-input"]').simulate('change', {target: {name: "friends-find-input", value: "Person2"}});
    expect(wrapper.find('input[name="friends-find-input"]').props().value).toEqual("Person2");
  });
  ...
  it ('should not send request when no user is given', () => {
    wrapper.find('input[name="friends-find-input"]').simulate('change', {target: {name: "friends-find-input", value: ""}});
    wrapper.find('button[name="friends-find-request"]').simulate('click');
    expect(mockedDispatch).not.toHaveBeenCalledWith(userRequestFriendship);
  });

  it ('should not send request when user given is self', () => {
    wrapper.find('input[name="friends-find-input"]').simulate('change', {target: {name: "friends-find-input", value: "Person"}});
    wrapper.find('button[name="friends-find-request"]').simulate('click');
    expect(mockedDispatch).not.toHaveBeenCalledWith(userRequestFriendship);
  });
