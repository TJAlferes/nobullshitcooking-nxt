...
describe('MessagesView', () => {
  ...
  it(`
    ...
    with className chat-message-list and
    with messagesRef as ref
  `, () => {
    expect(wrapper.find('ul.chat-message-list').prop('ref'))
      .toEqual(messagesRef);
  });

  it (`
    displays a starting message: a span element
    with className message--admin and
    with text 'COOK EAT WIN REPEAT'
  `, () => {
    expect(wrapper.find('span.message--admin').at(0).text())
  ...
  describe('when messages contain a status message', () => {
    ...
    `, () => {
      expect(wrapper.find('span.message--admin').at(1).text())
        .toEqual('Some status.');
    });
  });
  ...
  describe('when messages contain a sent public message', () => {
    it(`
      displays a span element
      with className message--self and
      with text 'Person: '
    `, () => {
      expect(wrapper.find('span.message--self').at(0).text())
        .toEqual('Person: ');
    });

    it(`
      displays an li element
      with className chat-message
      with text 'sometime Person: Hey all!'
    `, () => {
      expect(wrapper.find('li.chat-message').at(2).text())
  ...
  describe('when messages contain a received public message', () => {
    it(`
      displays a span element
      with className message--other and
      with text 'Person2: '
    `, () => {
      expect(wrapper.find('span.message--other').at(0).text())
        .toEqual('Person2: ');
    });
    ...
    `, () => {
      expect(wrapper.find('li.chat-message').at(3).text())
  ...
  describe('when messages contain a sent private message', () => {
    it(`
      displays a span element
      with className message--self
      and text 'You whisper to Person2: '
    `, () => {
      expect(wrapper.find('span.message--self').at(1).text())
        .toEqual('You whisper to Person2: ');
    });

    it(`
      displays a span
      with className message--private
      and text You good?
    `, () => {
      expect(wrapper.find('span.message--private').at(0).text())
        .toEqual('You good?');
    });
  });
  ...
  describe('when messages contain a received private message', () => {
    it(`
      displays a span element
      with className message--other
      with text 'Person2 whispers to you: '
    `, () => {
      expect(wrapper.find('span.message--other').at(1).text())
