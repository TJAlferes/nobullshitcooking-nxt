...
describe('RecipeView', () => {
  ...
  describe('content', () => {
    const wrapper = shallow(<RecipeView favorited={false} saved={false} userIsAuthenticated={false} {...initialProps} />);
    ...
    it('displays an em element with text A descriptive description.', () => {
      expect(wrapper.find('em.recipe-description').text()).toEqual("A descriptive description.");
    });
  ...
  describe('when user is authenticated', () => {
    ...
    describe ('when favorite not clicked', () => {
      const wrapper = shallow(<RecipeView favorited={false} saved={false} userIsAuthenticated={true} {...initialProps} />);
  
      it('displays a button with text Favorite', () => {
        expect(wrapper.find('button[name="favorite-button"]').text()).toEqual("Favorite");
      });
    });

    describe ('when favorite clicked', () => {
      const wrapper = shallow(<RecipeView favorited={true} saved={false} userIsAuthenticated={true} {...initialProps} />);
    ...
    describe ('when save not clicked', () => {
      ...
      it('displays a button with text Save', () => {
        expect(wrapper.find('button[name="save-button"]').text()).toEqual("Save");
      });
    });

    describe ('when save clicked', () => {
      const wrapper = shallow(<RecipeView favorited={false} saved={true} userIsAuthenticated={true} {...initialProps} />);
