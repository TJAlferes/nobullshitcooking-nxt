  ...
  const recipeBy = () => {
    ...
    return (
      <Link href={`/profile/${author}`}>
        <a className="recipe-author">{author}</a>
      </Link>
    );
  };
      ...
      <div className="two-col-b-left">
        ...
        <div className="recipe-image">
          {recipe_image !== "nobsc-recipe-default"
            ? <img src={`${url}/${recipe_image}`} />
            : <div className="img-280-172"></div>}
        </div>
        ...
        <div className="recipe-description-outer">
          ...
          <em className="recipe-description">{description}</em>
        </div>

        <div className="recipe-cuisine-outer">
          <b>Cuisine:</b>{' '}

          <span className="recipe-cuisine">{cuisine_name}</span>
        </div>

        <div className="recipe-type-outer">
          <b>Recipe type:</b>{' '}

          <span className="recipe-type">{recipe_type_name}</span>
        </div>
        ...
        <div className="recipe-equipment-image">
          {equipment_image !== "nobsc-recipe-equipment-default"
            ? <img src={`${url}-equipment/${equipment_image}`} />
            : <div className="img-280-172"></div>}
        </div>
        ...
        <div className="recipe-ingredients-image">
          {ingredients_image !== "nobsc-recipe-ingredients-default"
            ? <img src={`${url}-ingredients/${ingredients_image}`} />
            : <div className="img-280-172"></div>}
        </div>
        ...
        <div className="recipe-cooking-image">
          {cooking_image !== "nobsc-recipe-cooking-default"
            ? <img src={`${url}-cooking/${cooking_image}`} />
            : <div className="img-280-172"></div>}
        </div>
