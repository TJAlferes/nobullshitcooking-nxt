...
export function PublicRecipes({ activateModal, deactivateModal, deleteName, disownPublicRecipe, getApplicationNode, modalActive, myPublicRecipes, subTab, subTabClick }: Props): JSX.Element {
  return (
    <div className="dashboard-content">
      ...
      <Link href="/user-recipes/public/submit"><a className="new-entity">Create New Public Recipe</a></Link>
      ...
      {modalActive
        ? (
          ...
          >
            <p className="dashboard-prompt">{'Disown Recipe: '}{deleteName}{' ?'}</p>
            <button className="dashboard-modal__button--cancel" onClick={deactivateModal}>No</button>
            <button className="dashboard-modal__button--action" onClick={disownPublicRecipe}>Yes, Disown Recipe</button>
          </AriaModal>
        )
        : false
      }
      ...
      {myPublicRecipes.length
        ? myPublicRecipes.map(r => (
          <div className="dashboard-item" key={r.id}>
            ...
            <span className="dashboard-item-action"><Link href={`/user-recipe/public/edit/${r.id}`}><a className="dashboard-item__a">Edit</a></Link></span>
