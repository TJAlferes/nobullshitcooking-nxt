...
import Register from '../../../src/pages/register/index.page';
import { authUserRegister, authUserVerify } from '../../../src/store/auth/actions';
...
const initialProps = {confirmingUser: false, message: "Some message."};

let wrapper: ReactWrapper;

beforeEach(() => {
  useRouter.mockImplementation(() => ({route: "/register", pathname: "/register"}));
...
describe('Register', () => {
  it('should record and display changes to username', () => {
    wrapper.find('input[name="username"]').simulate('change', {target: {name: "username", value: "Person"}});
    expect(wrapper.find('input[name="username"]').props().value).toEqual("Person");
  });
  ...
  it('should record and display changes to passwordAgain', () => {
    wrapper.find('input[name="passwordAgain"]').simulate('change', {target: {name: "passwordAgain", value: "secret"}});
    expect(wrapper.find('input[name="passwordAgain"]').props().value).toEqual("secret");
  });
  ...
  it('should not submit when given passwords are different', () => {
    ...
    wrapper.find('input[name="email"]').simulate('change', {target: {name: "email", value: "person@place.com"}});
    wrapper.find('input[name="password"]').simulate('change', {target: {name: "password", value: "secret"}});
    wrapper.find('input[name="passwordAgain"]').simulate('change', {target: {name: "passwordAgain", value: "secre"}});
    wrapper.find('#create_account_button').at(1).simulate('click');
    expect(mockedAuthUserRegister).toBeCalledTimes(0);
  });

  it('should not submit when username is less than 2 characters', () => {
    wrapper.find('input[name="username"]').simulate('change', {target: {name: "username", value: "P"}});
    wrapper.find('input[name="email"]').simulate('change', {target: {name: "email", value: "person@place.com"}});
    wrapper.find('input[name="password"]').simulate('change', {target: {name: "password", value: "secret"}});
    wrapper.find('input[name="passwordAgain"]').simulate('change', {target: {name: "passwordAgain", value: "secret"}});
    wrapper.find('#create_account_button').at(1).simulate('click');
